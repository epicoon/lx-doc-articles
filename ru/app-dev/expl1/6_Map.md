[Английская версия (English version)](https://github.com/epicoon/lx-doc-articles/en/app-dev/expl1/6_Map.md)

### Шаг 6. Создание карты игровой поверхности

На текущем шаге и следующем будем создавать классы, которые помогут лучше структурировать код. Создадим для этих классов каталог `service/tetris/module/game/frontend/classes`.

Добавим класс `Map` в пространство имен `tetris`. Он будет ответственен за игровую поверхность. Займется подсвечиванием ячеек, поиском свободных ячеек для фигур, определением заполнения рядов. Для этого создадим в каталоге `service/tetris/module/game/frontend/classes` файл `Map.js`.

Объявим класс и конструктор. Конструктор в качестве аргумента будет принимать виджет, представляющий визуализацию игровой поверхности. Также пригодятся поля `colsCount`и `rowsCount`, чтобы лишний раз не лазить в виджет. Поле `rows` будет содержать массив, с помощью которого будет отслеживать заполнение рядов.
```js
...
```js
	checkFilled() {
		...
		tetris.Game.instance.addFilled(filled);
	}
```
...
[Следующий шаг](https://github.com/epicoon/lx-doc-articles/ru/app-dev/expl1/7_Timer.md)
